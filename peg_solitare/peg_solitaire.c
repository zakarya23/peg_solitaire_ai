...
void printToFile(FILE *fptr, double cpu_time_used) {
	fprintf(fptr,"STATS: \n");
		fprintf(fptr,"\tExpanded nodes: %'d\n\tGenerated nodes: %'d\n", expanded_nodes, generated_nodes);
		fprintf(fptr,"\tSolution Length: %d\n", solution_size);
		fprintf(fptr,"\tNumber of Pegs Left: %d\n", num_pegs( &(solution[solution_size]) ) );
		fprintf(fptr,"\tExpanded/seconds: %d\n", (int)(expanded_nodes/cpu_time_used) );
		fprintf(fptr,"\tTime (seconds): %f\n", cpu_time_used );
...
int main(int argc, char *argv[]) {
	...
	if( ai_run ){ 
		...
		printToFile(fptr, cpu_time_used); 
